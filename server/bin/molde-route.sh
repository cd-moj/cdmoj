#!/bin/bash
# molde-route.sh {add|rm|ls} <rota> — liga/desliga UMA rota da API no pool do MOLDE.
#   rota = caminho sob /api/v1, ex.: contest/updates
# `add` escreve /etc/nginx/moj-molde-routes/<slug>.conf (location exata apontando p/ o socket
# do molde, com anteparo+breaker+microcache+shed replicados e fallback 502 → fcgiwrap),
...
cat > "$F" <<EOF
# rota no MOLDE (gerado por molde-route.sh — rm deste arquivo + reload = volta ao fcgiwrap)
location = /api/v1/$ROTA {
    ...
    fastcgi_pass            unix:$WORKROOT/run/moj-molde.sock;
...
echo ">> rota $ROTA no molde (fallback 502 → fcgiwrap armado)"
