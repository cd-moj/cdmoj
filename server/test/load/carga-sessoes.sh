...
: "${CARGA_TMP:=/tmp}"   # bancada local grava os tokens no workdir dela
...
: > "$CARGA_TMP"/carga-tokens-teams.txt; : > "$CARGA_TMP"/carga-tokens-staff.txt
mk(){ local tok="zc$(od -An -N15 -tx1 /dev/urandom | tr -d ' \n')"
  printf 'CONTEST=%q\nLOGIN=%q\nUSERFULLNAME=%q\nLOGINAT=%q\n' "$C" "$1" "$1" "$EPOCHSECONDS" > "$RUNDIR/sessions/$tok"
  echo "$tok" >> "$2"; }
for d in "$CD"/users/eq-*/;     do u="${d%/}"; mk "${u##*/}" "$CARGA_TMP"/carga-tokens-teams.txt; done
for d in "$CD"/users/s*.staff/; do u="${d%/}"; mk "${u##*/}" "$CARGA_TMP"/carga-tokens-staff.txt; done
wc -l "$CARGA_TMP"/carga-tokens-teams.txt "$CARGA_TMP"/carga-tokens-staff.txt
