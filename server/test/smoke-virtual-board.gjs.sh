...
{ printf 'const FEED = %s;\n' "$(cat "$W/feed.json")"
  ...
  printf 'const TXT_OF = %s;\nconst TXT_CV = %s;\n' "$(jq -Rs . "$FIX/dfull/var/placar-view-oficial.txt")" "$(jq -Rs . "$FIX/dfull/var/placar-view-conv.txt")"
...
function cmp(label, txt, t, teamOk) {
  const want = parseTxt(txt); const idx = indexFeed(FEED); const got = boardAt(FEED, idx, [], t, {}, { teamOk }).teams;
  let bad = [];
  ...
  for (const w of want) { const g = G[w.u]; if (!g) { bad.push('falta ' + w.u); continue; }
    if (!teamOk && g.guest !== w.guest) bad.push(w.u + ' guest');
    if (!(teamOk && g.guest) && g.place !== w.place) bad.push(w.u + ' place ' + g.place + ' != ' + w.place);
...
// COORTE recortada no motor == placar PRÓPRIO daquela visão no servidor (posição e ★ inclusive)
print('views=' + JSON.stringify((FEED.views || []).map((v) => v.id)) + ' coh=' + FEED.teams.map((tm) => tm[6]).join(','));
cmp('view_oficial', TXT_OF, Infinity, (tm) => tm[6] === 'oficial');
cmp('view_conv', TXT_CV, Infinity, (tm) => tm[6] === 'conv');
// recorte de LINHA: a virtual ganha a posição ENTRE OS VISÍVEIS
{ const ix = indexFeed(FEED); const VV = [{ login: 'ana', name: 'Ana', runs: [[3000, 0, 'Y']], you: true }];   // 1 problema aos 50 min: atrás de t1,t2 (2) e de t3,t4 (1 aos 40)
  const b2 = boardAt(FEED, ix, VV, Infinity); const keep = (x) => x.you || x.username === 't5';
  const before = myRow(b2).gplace; sliceVirtualPlaces(b2, keep);
  print('slice_gplace=' + before + '->' + myRow(b2).gplace); }
...
check view_oficial ok   "coorte 'oficial' no motor == placar-view-oficial.txt (posição e ★ da visão)"
check view_conv ok      "coorte 'conv' no motor == placar-view-conv.txt"
[[ "$(kv views)" == '["oficial","conv"] coh=oficial,oficial,oficial,oficial,oficial,conv' || "$(kv views)" == *'"oficial"'*'"conv"'* ]] && { PASS=$((PASS+1)); echo "  ok: feed traz views + coorte por time"; } || { FAIL=$((FAIL+1)); echo "  FALHOU: views/coorte no feed ($(kv views))"; }
check slice_gplace "5->1" "recorte de linha: virtual sobe p/ a posição entre os visíveis"
