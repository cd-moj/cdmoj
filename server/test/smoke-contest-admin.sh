...
echo "== Sonic liga/desliga; {} não mexe; null volta ao padrão =="
call /contest/balloons GET '' cadm 'contest=ac-c'
ck "rota pública vê Sonic ligado"  '[[ "$(jq -r ".balloons.enableSonic" <<<"$BODY")" == true && "$(jq -r ".balloons.A" <<<"$BODY")" == 00FF00 ]]'
call /contest/admin/config POST '{"colors":{"A":"00FF00","enableSonic":false}}' cadm 'contest=ac-c'
call /contest/balloons GET '' cadm 'contest=ac-c'
ck "desligou o Sonic: rota mostra false e a cor ficou" '[[ "$(jq -r ".balloons.enableSonic" <<<"$BODY")" == false && "$(jq -r ".balloons.A" <<<"$BODY")" == 00FF00 ]]'
call /contest/admin/config POST '{"colors":{}}' cadm 'contest=ac-c'
ck "colors:{} NÃO apaga o balloons.json" '[[ "$(jq -r .A < "$FIX/ac-c/balloons.json")" == "00FF00" ]]'
call /contest/admin/config POST '{"colors":null}' cadm 'contest=ac-c'
ck "colors:null apaga (volta ao padrão)" '[[ ! -e "$FIX/ac-c/balloons.json" ]]'
call /contest/balloons GET '' cadm 'contest=ac-c'
