...
#
# Estatísticas contam SÓ usuários normais — descarta privilegiados (.admin/.judge/.staff/.mon).
set -u
...
for (( i=0; i<${#PROBS[@]}; i+=5 )); do
  praw="${PROBS[$((i+1))]}"
  pm_items+=( "$(jq -cn --arg off "$i" --arg raw "$praw" --arg dot "${praw/\//.}" \
      --arg short "${PROBS[$((i+3))]}" --arg full "${PROBS[$((i+2))]}" \
      '{off:$off, raw:$raw, dot:$dot, short:$short, full:$full}')" )
done
