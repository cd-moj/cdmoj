...
# competidor vê é o admin/juiz-chefe: conf **`STATEMENT_LANGS`** (lista; ausente = `pt`). Os
# arquivos ficam em `enunciados/<skey>.html` (PT, como sempre) e `enunciados/<skey>.<lang>.html|pdf`.
# Idioma pedido sem arquivo cai no PT (nunca 404 por falta de tradução — só por falta de direito).
#
#   cs_norm <raw>                 -> normaliza uma lista ("pt en", "en,es") p/ a allowlist; vazio = pt
#   cs_langs <contest>            -> idiomas OFERECIDOS (conf STATEMENT_LANGS), "pt" se ausente
...
cs_norm(){
  ...
  for l in $raw; do l="${l,,}"; stmt_lang_ok "$l" || continue; [[ " $out " == *" $l "* ]] || out+="${out:+ }$l"; done
  [[ -n "$out" ]] || out=pt
  printf '%s' "$out"
}
