...
sup=false; is_superadmin && sup=true   # só super-admin cria id `icpc*` (reserved_id_prefixes)
ok_json '{can_create:$c, is_admin:$adm, is_superadmin:$sup, reserved_id_prefixes:["icpc"], reason:$r, solved_count:$s, threshold:$t, in_allow:$ia, in_deny:$idn, allowed_modes:$modes, login:$lg, name:$nm}' \
  --argjson c "$can" --argjson adm "$CC_ISADMIN" --argjson sup "$sup" --arg r "$CC_REASON" \
