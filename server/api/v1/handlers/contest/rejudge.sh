...
# (users/<login>/submissions/<id>.<lang> no store-v2; flat no legado) + metadados do
# history e reinjeta no spool como uma SUBMISSÃO normal (mesmo id), marcando a linha como
# pendente. Assim funciona com o daemon que JÁ está rodando (caminho de submit) — sem
# depender de marcador vazio nem de reiniciar o daemon. Reporta as puladas (sem fonte/linha).
require_method POST
