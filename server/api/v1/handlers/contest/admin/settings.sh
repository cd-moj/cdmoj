...
bset(){ # <jsonkey> <VAR> <on-value-p/-positivos>
  ...
  if [[ "$on" == true ]]; then
    case "$2" in LOGIN_ENABLED|SHOWLOG|SHOWEDITOR) delvar "$2";; *) setvar "$2" "$3";; esac
  else
    case "$2" in LOGIN_ENABLED) setvar LOGIN_ENABLED n;; SHOWLOG) setvar SHOWLOG 0;; SHOWEDITOR) setvar SHOWEDITOR 0;; *) delvar "$2";; esac
  fi
}
