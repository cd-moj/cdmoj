...
# heartbeat, então restart nunca sobe com partição divergente (C5 do incidente 2026-07-15).
require_method POST
require_worker
...
# BOOT do agente: devolve à fila o que estava atribuído a este host (restart não perde fila).
# Só no boot — registers de rotina (pós-update/gc) acontecem com slots ATIVOS e re-enfileirar
# duplicaria trabalho vivo.
[[ "$boot" == true ]] && sched_requeue_host "$host"

ok_json '{host:$h, registered:true, ttl:$ttl, config:$cfg}' \
  --arg h "$host" --argjson ttl "$REG_TTL" --argjson cfg "$(judges_config_for "$host")"
