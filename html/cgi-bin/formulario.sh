...
if [[ "x$(< $TMP)" != "x" ]]; then
  ...
  TIPO="$(grep -a -A3 'tipo_contest' "$POST"|tr '\n' '&'|cut '-d&' -f3|tr -d '\r')"
  ...
  PESO="$(grep -a -A2 'peso_problema' "$POST"|sed -e 's/\--/@/g'|tr '\n' '&'|tr -d '\r')"
  PENALIDADE="$(grep -a -A2 'peso_penalidade' "$POST"|sed -e 's/\--/@/g'|tr '\n' '&'|tr -d '\r')"
  ...
  while (("$cont" <= "$N_CONTESTS")); do
    ...
    prob+="$(echo $PESO|sed -e 's/@&/@/g'|cut '-d@' -f"$cont"|cut '-d&' -f3) "
    prob+="$(echo $PENALIDADE|sed -e 's/@&/@/g'|cut '-d@' -f"$cont"|cut '-d&' -f3) "
