...
window.renderSubmissionsTable = function(subs, filterProb, problems, allowLog) {
    ...
    for (let s of subsF) {
        ...
        if (/^(not\s*answered\s*yet|on\s*queue|running)$/i.test((s.verdict || "").trim())) {
            ...
            if(window.balloonColors["enableSonic"])
                cellContent=`<span>${getSubmissionIcon("pending",s.epoch)}${s.verdict}</span>`
        }
        const prob = problems.find(p=>p.problem_id===s.probid) || {};
        const fileLink = `<a class="link-btn" title="Baixar código fonte"
      href="#" onclick="downloadAuthenticated('${window.API_SOURCE}/${contestID}/submission/source/${encodeURIComponent(s.subid)}/${encodeURIComponent(s.epoch)}/','${s.filename}');return false;">${s.filename}</a>`;
...
window.fetchAndRenderSubmissions = async function() {
    ...
    if (!user) return;
    const headers = localStorage.getItem(TOKEN_KEY)
          ? {'Bearer': localStorage.getItem(TOKEN_KEY)} : {};
    //  const resp = await fetch(window.API_SUBMISSIONS+encodeURIComponent(user), {headers});
    const resp = await fetch(`${window.API_SUBMISSIONS}/${contestID}/full/history/`, {headers});
...
window.renderSubmissionsGeneral = function() {
    let bar = `<span class="subfilter-btn${window.subFilter==="ALL"?' active':''}" data-pb="ALL">Todos</span>`;
    for(const p of window.contestProblems) {
        if(p.show!==false)
            bar += `<span class="subfilter-btn${window.subFilter===p.problem_id?' active':''}" data-pb="${p.problem_id}">${p.short_name}</span>`;
    }
...
window.downloadAuthenticated = function(url, filename) {
    fetch(url, { headers: { 'Bearer': localStorage.getItem(TOKEN_KEY) } })
