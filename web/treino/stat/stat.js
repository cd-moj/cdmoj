...
const DOW_LABELS = [T('Dom', 'Sun'), T('Seg', 'Mon'), T('Ter', 'Tue'), T('Qua', 'Wed'), T('Qui', 'Thu'), T('Sex', 'Fri'), T('Sáb', 'Sat')];
...
function renderQuickStats(stats) {
  ...
  box.append(
    ...
    card(taxa + '%', T('taxa de acerto (AC / total)', 'acceptance rate (AC / total)')),
    card(mediaTent, T('tentativas médias até resolver', 'average attempts to solve')),
    card(firstTryPct + '%', T('acerto na 1ª tentativa', 'solved on first try')),
    card(curStreak + (curStreak === 1 ? T(' dia', ' day') : T(' dias', ' days')), T('streak atual', 'current streak')),
    card(maxStreak + (maxStreak === 1 ? T(' dia', ' day') : T(' dias', ' days')), T('maior streak', 'longest streak')),
    card(nLangs, T('linguagens usadas', 'languages used')),
    card(mostUsed || '—', T('linguagem mais usada', 'most used language')),
    card(bestLang, T('linguagem mais eficaz', 'most effective language')),
    card(busiest, T('dia mais ativo', 'busiest day')),
  );
}
...
function renderDashboard(stats) {
  ...
  {
    const sec = sectionCard(T('📈 Desenvolvimento ao longo do tempo', '📈 Progress over time'));
    sec.append(el('div', { class: 'chart-title' }, T('Problemas distintos resolvidos (acumulado)', 'Distinct problems solved (cumulative)')));
    if (solvedDates.length) {
      ...
      sec.append(el('div', { class: 'muted small center' }, T('Nenhum problema resolvido ainda.', 'No problems solved yet.')));
    }
    dash.append(sec);
  }
  ...
  {
    const sec = sectionCard(T('🔥 Atividade diária (últimas 26 semanas)', '🔥 Daily activity (last 26 weeks)'));
    sec.append(el('div', { class: 'muted small', style: 'margin-bottom:.5rem' }, T('Cada quadrado é um dia; quanto mais escuro, mais submissões.', 'Each square is a day; the darker, the more submissions.')));
  ...
  {
    const sec = sectionCard(T('🗓️ Submissões por dia', '🗓️ Submissions per day'));
    sec.append(el('div', { class: 'chart-title' }, T('Últimos 45 dias', 'Last 45 days')));
    const days = [];
    const now = new Date();
    for (let i = 44; i >= 0; i--) {
  ...
  {
    const sec = sectionCard(T('🎯 Veredictos e ritmo semanal', '🎯 Verdicts and weekly rhythm'));
    const grid = el('div', { class: 'stat-grid two' });
    // pizza de veredictos
    const vEntries = Object.entries(stats.verdictStats).sort((a, b) => b[1] - a[1]);
    const vBox = el('div', {}, el('div', { class: 'chart-title' }, T('Distribuição de veredictos', 'Verdict distribution')));
    ...
    const dowBox = el('div', {}, el('div', { class: 'chart-title' }, T('Submissões por dia da semana', 'Submissions by day of week')));
  ...
  {
    const sec = sectionCard(T('💻 Desempenho por linguagem', '💻 Performance by language'));
    const langs = Object.keys(stats.langStats)
      .sort((a, b) => stats.langStats[b].total - stats.langStats[a].total);
    if (!langs.length) {
      sec.append(el('div', { class: 'muted small' }, T('Sem dados de linguagem.', 'No language data.')));
    } else {
      sec.append(el('div', { class: 'muted small', style: 'margin-bottom:.5rem' },
        T('Onde você mais acerta: taxa de AC por linguagem (mín. 3 submissões em destaque).', 'Where you succeed most: AC rate by language (min. 3 submissions highlighted).')));
      const grid = el('div', { class: 'stat-grid two' });
      ...
      const table = el('table', { class: 'moj' },
        el('thead', {}, el('tr', {}, el('th', {}, T('Linguagem', 'Language')), el('th', {}, 'Subs'), el('th', {}, 'AC'), el('th', {}, T('Taxa AC', 'AC rate')))),
        tb);
      grid.append(el('div', {}, el('div', { class: 'chart-title' }, T('Resumo', 'Summary')), table));
      ...
      const barBox = el('div', {}, el('div', { class: 'chart-title' }, T('Taxa de acerto por linguagem (%)', 'Acceptance rate by language (%)')));
      if (rateBars.length) barBox.append(barChart(rateBars, { width: 360, height: 240, color: '#1a7f37' }));
      else barBox.append(el('div', { class: 'muted small center', style: 'padding:1rem' }, T('Poucas submissões para comparar.', 'Too few submissions to compare.')));
      grid.append(barBox);
  ...
  {
    const sec = sectionCard(T('🏷️ Forças por tag', '🏷️ Strengths by tag'));
    const tags = Object.keys(stats.tagStats);
    if (!tags.length) {
      sec.append(el('div', { class: 'muted small' }, T('Sem tags (problemas sem metadados de tag).', 'No tags (problems without tag metadata).')));
    } else {
      sec.append(el('div', { class: 'muted small', style: 'margin-bottom:.5rem' }, T('Áreas em que você mais resolve problemas (distintos).', 'Areas where you solve the most (distinct) problems.')));
      const grid = el('div', { class: 'stat-grid two' });
      const topByResolved = tags.filter(t => stats.tagStats[t].resolved > 0)
        ...
        .slice(0, 12);
      const barBox = el('div', {}, el('div', { class: 'chart-title' }, T('Resolvidos por tag (top 12)', 'Solved by tag (top 12)')));
      if (topByResolved.length) {
        ...
        barBox.append(el('div', { class: 'muted small center', style: 'padding:1rem' }, T('Nenhum resolvido ainda.', 'None solved yet.')));
      }
      const pieBox = el('div', {}, el('div', { class: 'chart-title' }, T('Participação por tag (resolvidos)', 'Share by tag (solved)')));
  ...
  {
    const sec = sectionCard(T('📜 Histórico completo de submissões', '📜 Full submission history'));
...
async function downloadAuthed(path, filename) {
  ...
  } catch { alert(T('Falha ao baixar.', 'Failed to download.')); }
}
...
function renderHistory() {
  ...
  box.innerHTML = '';
  if (!rows.length) { box.innerHTML = '<span class="muted small">' + T('Nenhuma submissão.', 'No submissions.') + '</span>'; return; }
  ...
  const head = el('thead', {}, el('tr', {},
    th(T('Data/Hora', 'Date/Time'), 'date'), th(T('Problema', 'Problem'), 'problem'), th(T('Linguagem', 'Language'), 'lang'),
    canLog ? el('th', {}, T('Cód/Log', 'Code/Log')) : null, th('Status', 'status')));
  const tb = el('tbody');
  rows.forEach(s => {
    const pending = isPending(s.verdict);
    const logTd = canLog ? el('td', { class: 'small' },
      el('a', { href: '#', onclick: (e) => { e.preventDefault(); downloadAuthed(`/submission/source?contest=${CONTEST}&id=${encodeURIComponent(s.subid)}&time=${encodeURIComponent(s.epoch)}`, s.subid + '.txt'); } }, T('cód', 'code')),
      ' · ',
...
function avatarEl(profile) {
  ...
  if (profile.has_photo) {
    return el('img', { class: 'avatar', alt: T('foto de ', 'photo of ') + login,
...
async function editorRankLine(editorId) {
  ...
  return `Editor: ${label} — ` +
    T(`${rank}º mais usado (de ${list.length})`, `#${rank} most used (of ${list.length})`) +
    `, ${ppl} ` + (ppl === 1 ? T('pessoa', 'person') : T('pessoas', 'people')) +
    (total ? ` · ${total} ` + T('no total', 'total') : '');
}
...
async function renderProfileHeader(profile) {
  ...
  if (profile.university) meta.append(el('div', { class: 'pline' }, el('span', { class: 'lbl' }, T('Universidade: ', 'University: ')), profile.university));
...
async function boot() {
  ...
  document.title = T('Estatísticas de ', 'Statistics of ') + USER + ' — MOJ';
  ...
  try {
    ...
    document.getElementById('profileHead').innerHTML =
      '<div class="error-box">' + T('Não foi possível carregar este perfil: ', 'Could not load this profile: ') + (e.message || T('erro', 'error')) + '</div>';
    return;
  }
