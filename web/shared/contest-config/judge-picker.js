...
export function makeJudgePicker(selectedHosts, apiCtx) {
  ...
  const box = el('div', { class: 'lang-grid' }, el('span', { class: 'muted small' }, 'carregando juízes…'));
  const boxes = [];           // { host, c }
  ...
  (async () => {
    try {
      ...
      [...sel].filter((h) => !js.some((j) => j.host === h))
        .forEach((h) => box.append(addBox(h, '⚠️ não registrado')));
      if (!boxes.length) box.append(el('span', { class: 'muted small' }, 'nenhum juiz registrado'));
    } catch {
      // degrada p/ texto livre — não bloqueia a criação/edição do contest
      fallback = el('input', { value: [...sel].join(' '), placeholder: 'hosts separados por espaço (vazio = todos)', style: 'width:100%' });
      box.replaceChildren(el('span', { class: 'muted small' }, 'não deu para listar os juízes — informe os hosts:'), fallback);
    }
  })();
