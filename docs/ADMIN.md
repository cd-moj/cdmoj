...
- **Chave do webcast** (`mojwc_…`) — NÃO se gera à mão: sai da página `/contest/animeitor/`
  ...
  mesma tela gere as fotos e as músicas dos times. A **sede** também abre essa tela, recortada
  pelo `staff-filters.json` (Pessoas › Sedes & escolas) e sem as chaves: o `.cstaff` sobe/troca as
  fotos e músicas da sede dele, o `.staff` só olha e ouve.
  Ver `docs/WEBCAST.md`.
