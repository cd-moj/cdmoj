...
| `/problems/status` | GET | **painel** dos problemas do login (dono+colaborador; privado de terceiro **não** aparece — `owners_visible`): `{total,counts:{validated,validation_error,unvalidated,calibrated,uncalibrated,being_calibrated,needs_recalibration,errors},calibrating_ids,attention_ids,problems:[{id,title,owner,author,public,validated,calibrated,being_calibrated,stale,needs_recalibration,error,error_reasons,time_limits,updated_at}]}`. `stale`/`needs_recalibration` do checksum carimbado no índice (≤30 min de atraso); **sem hash de pacote por request** |
