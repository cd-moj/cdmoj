...
| `/treino/admin/judges` | GET | máquinas de juiz (modelo pull) `{online,busy,machines:[{host,online,busy,langs,cage_root,cache,tl,current,queued_calibrate,report}]}` — `current` = job em execução `{kind:"submission"\|"calibrate"\|"index"\|"unknown_busy",problem_id,…}` ou `null`; `queued_calibrate` = calibrações direcionadas na fila do host; `report.gpu` = GPU de **compute** comprovada (`{vendor:nvidia\|amd,names}`, do `nvidia-smi`/`rocm-smi`) ou `null` |
