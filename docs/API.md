...
| Rota | Auth | I/O |
|---|---|---|
| `/contest/basic?contest=<c>` | — | `{contest_id,contest_name,start_time,end_time,login_start_time,locale,login_enabled,freeze_time,score_anon,languages[],secret}` (`languages` = whitelist do conf `LANGUAGES=`; `[]` = todas). **Continua público mesmo em contest secreto** — a tela de login/countdown precisa do nome p/ quem tem o link. Com Bearer de sessão **deste** contest (opcional), `end_time` é o **fim EFETIVO do login** (prorrogação por sede/grupo via `time-overrides.json` — o countdown mostra o certo) |
...
| `/contest/badges?contest=<c>[&staff=<l>&include_disabled=1]` | GET/POST | Bearer (`.staff`/admin) | **credenciais p/ etiquetas imprimíveis** (página `/contest/badges/`, gabaritos Pimaco A4). GET → `{users:[{login,name,fullname,password,disabled,region,univ}], staff:[…só admin], regions, staff_password, contest_name, start_epoch}` — ordenado por região; `name` = `.team.name`→`fullname`; `region` = **`.team.region` explícita** quando setada, senão casada por regex de `regions.json` (e o recorte do staff entende `region:<nome>`). **Escopo forçado pela API**: `.staff` recebe só o próprio recorte (`staff-filters.json`; vazio = tudo) + a própria conta (`staff=` alheio → `403 staff_scope`); admin vê tudo ou o arquivo de uma sede via `staff=<l>`. Contas `.admin/.judge/.cjudge/.mon` nunca entram. POST (só admin) `{staff_password:bool}` liga/desliga a variante **com senha** p/ o `.staff` — desligada, o GET do staff vem **sem o campo `password`**. É o **único** endpoint que devolve senha em claro numa releitura — **toda chamada é auditada** (`badges-view`/`badges-config`) |
...
| `/contest/admin/user-add?contest=<c>` | POST | `{login,password?,fullname?,email?, team_name?,univ_short?,univ_full?,country?,region?}` → adiciona/reseta, devolve a credencial; os campos de TIME mesclam no `.team` do account.json |
| `/contest/admin/users-bulk?contest=<c>` | POST | **carga em lote** `{users:[{login,password?,fullname?,email?, team_name?,univ_short?,univ_full?,country?,region?}], on_existing?:skip\|update}` (default `skip`; ≤5000; senha vazia = gerada). Os campos de TIME (opcionais) gravam o `.team{name,univ_short,univ_full,flag,region}` — **carga única** de credenciais+país+sede+universidade (a UI aceita CSV com cabeçalho: `login,senha,nome,time,pais,sede,univ,univ_nome`, ordem livre). `update` troca senha/nome/email de existentes **exceto conta privilegiada** (`.admin/.judge/.cjudge/.staff/.mon` — nunca resetada em massa; skip `privileged`); criar privilegiada nova é permitido. `passwd` reescrito 1× (v2: `account.json` + 1 `regen_passwd`). → `{created:[{login,password,fullname,email}],updated:[…],skipped:[{login,reason:exists\|privileged\|invalid\|duplicate}],counts}`. Auditado `users-bulk` |
...
| `/contest/score?contest=<c>` | (Bearer opcional) | **TXT** (1ª linha = modo) — ver `SCOREBOARD.md`. Cache preguiçoso: (re)gera `placar.txt` (público, **com freeze**) e `placar-full.txt` (completo, **sem freeze**) se `history`/`conf` mudou. **Privilegiados** (`.admin`/`.judge`/`.cjudge` + allowlist `SCORE_FULL_USERS`) com token recebem o completo; demais, o público. `&view=public` força a visão **pública/congelada** mesmo p/ privilegiado — é o que a **cerimônia de revelação** (`/contest/score/reveal.html`, estilo ICPC resolver, nativa) usa p/ computar o delta frozen→full e revelar de baixo p/ cima; o botão "Descongelar tudo" da cerimônia = settings `freeze:0`. Em contest **SUPER SECRETO** (conf `SECRET=1`) o placar deixa de ser público: sem sessão **daquele** contest → **401** `secret_login_required` (idem `balloons`/`regions`/`teams-meta`). |
...
| `/treino/contest-create/create` | POST | Bearer+criador | `{id?,name,mode,priority?,start?,end,languages?,showcode?,allow_empty?, admin:{login?,password?,fullname?}, (users_from? \| users:[{login,password?,fullname?,email?, team_name?,univ_short?,univ_full?,country?,region?}]), problems:[…], colors?:{A:"RRGGBB",…,enableSonic?}, regions?:[…], teams_meta?:[{regex,country,school?,school_full?}], locale?,login_start?,login_enabled?,freeze?, show_log?,show_editor?,show_tl?,allow_backup?,allow_print?,score_anon?,manual_verdict?,allow_late?,secret?,login_ua_substring?,score_full_users?,penalty_minutes?,penalty_verdicts?}` → `{contest_id,admin_login,admin_reused,admin_password,users[],users_from,url,scoreboard_url}`. **Paridade com o settings**: os toggles/opções espelham `/contest/admin/settings` (grava só o não-default). `languages` aceita **array de ids canônicos** (normaliza como o settings) ou string legada. `priority` = prioridade no escalonador (prova/lista-privada/lista-publica; `super` só admin — como `mode:outro`). Por problema: `languages[]` (vira `problem-langs.json`) e `statement_pdf_b64`/`statement_pdf_file` (além do HTML). Admin **não** é sobrescrito: senha digitada é respeitada; em modo compartilhado, se o `<login>.admin` já existe na fonte `users_from` ele é **reutilizado** — `admin_reused:true`, `admin_password:null` |
...
| `/contest/admin/staff-filters?contest=<c>` | GET/POST | admin | escopo dos usuários `.staff` (sedes distribuídas): cada entrada é **`region:<nome>`** (igualdade com a sede `.team.region` do aluno — sem regex) **ou** uma regex no login (clássico). GET → `{staff:[{login,fullname,disabled}], filters:{login:[entradas]}, regions:[{name,regex}]}` (regions p/ semear — a UI insere `region:<nome>`). POST `{filters:{login:[entradas]}}` (chaves = `.staff` existentes; vazio = vê tudo) → grava `print-requests/staff-filters.json`. **Auditado** (`staff-filters`) |
| `/contest/admin/teams?contest=<c>` | POST | admin | **gerência de times por-usuário** (aba 👥 Times). `{set:{<login>:{team_name?,univ_short?,univ_full?,country?,region?}}}` mescla campos no `.team` (`""` **apaga** o campo; ausente não toca; login inexistente → `skipped`); `{action:"materialize"}` = **match de 1 clique**: aplica teams-meta (regex→país/escola) e regions (regex→nome de sede) aos campos **vazios** de cada usuário, gravando por-usuário (nunca sobrescreve preenchido) → `{materialized,filled}`. Contest com `USERS_FROM` → **409** `shared_users`. **Auditado** (`teams-set`/`teams-materialize`) |
...
| `/contest/admin/user-disable?contest=<c>` | POST | admin | `{login}` → bloqueia (senha `!…`) + desloga (reabilita via user-add). Conta privilegiada (`.admin/.judge/.cjudge/.staff/.mon`) → 403 |
| `/contest/admin/users-set-password?contest=<c>` | POST | admin | `{password,include_disabled?}` → senha única p/ todos os não-privilegiados (prova; pula `.admin/.judge/.cjudge/.staff/.mon`) |
