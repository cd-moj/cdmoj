...
| `/contest/admin/settings?contest=<c>` | GET/POST | admin | tempos, login on/off, abertura, **freeze**, locale, toggles `show_code/show_log/show_editor/show_tl/allow_late/score_anon/allow_backup/allow_print/manual_verdict/secret`, `login_ua_substring`, `languages[]` (whitelist do contest), `score_full_users[]` (logins que veem o placar completo além de `.admin`/`.judge`/`.cjudge`). `secret` = **SUPER SECRETO** (fora das listagens públicas; placar/visual exigem login no contest; a UI exige digitar o id p/ desmarcar). `manual_verdict` (opt-in, default OFF) liga o **veredicto manual**: o daemon SEGURA o veredicto computado p/ revisão de 2 juízes (exceto o que a matriz `auto-verdicts` libera) |
