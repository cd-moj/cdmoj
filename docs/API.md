...
| `/treino/problem?id=<id>` | — | `{id,title,author,statement_html_b64,time_limits,tags,collections,languages}` (`author` = arquivo `author` do pacote, verbatim; vários autores juntados por `, `; vazio se ausente; `collections` = coleções do `.moj-meta.json`; `languages` = ids de linguagem de submissão permitidos deste problema, `[]` = todas — o front do treino filtra o dropdown por essa lista) |
...
| `/treino/admin/judges` | GET | máquinas de juiz (modelo pull) `{online,busy,machines:[{host,online,busy,langs,cage_root,cache,tl,current,current_jobs,queued_calibrate,slots,partition,topology,config,report}]}` — `current_jobs` = TODOS os jobs em execução (multi-slot; `current` = o 1º, compat); `slots:{free,total}`; `partition` = vigente no agente; `config` = a config DESEJADA (judges-config) ou null; `report.gpu` = GPU de **compute** comprovada (`{vendor:nvidia\|amd,names}`) ou `null` |
