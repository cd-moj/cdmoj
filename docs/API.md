...
| `/contest/admin/bank?contest=<c>&q=&limit=&collection=` | GET | admin | busca no **banco público** do treino p/ adicionar problemas depois de criado `{problems:[{id,title,tags,collections}],total}`. `?meta=1` → `{tags:[{tag,count}],collections:[{collection,count}]}` (alimenta o painel de sorteio). **Só públicos** — privado entra apenas pelo `add` por id (gate de dono) |
