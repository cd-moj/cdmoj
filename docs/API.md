...
| `/contest/review/list?contest=<c>` | GET | judge | fila de revisão manual `{manual, options, items:[{id,login,problem_id,lang,computed_verdict,status,conflict,claimants:[{by,elapsed_s,expires_in_s}],votes_n,my_vote,votes(só chief)}], counts:{not_evaluated,being_evaluated,conflicts}, my_active}` |
